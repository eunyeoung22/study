...
model.add(Dense(100, activation='relu'))
model.add(Dense(100, activation='relu'))
model.add(Dense(10, activation='softmax')) # 아웃풋 마지막 개수는 y클래스 개수
...
filepath = './_save/MCP/' #파일 저장 경로 잡아 주기(경로 밑 폴더 먼저 생성해주기)
filename = '{epoch:04d}-{val_loss:.4f}.hdf5' #저장파일 확장자".hdf5"

mcp = ModelCheckpoint(monitor='val_loss', mode='auto', verbose=1,  #ModelCheckpoint : callbacks함수(가중치 저장)
                      save_best_only=True, 
