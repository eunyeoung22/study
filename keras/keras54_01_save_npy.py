...
xy_train = train_datagen.flow_from_directory(
    ...
    # class_mode='categorical',# 원핫(배의 메모리 차지하므로 binary로 하는게 성능에 좋을수 있음)
...
np.save('./_data/brain/brain_x2_train.npy', arr = xy_train[0][0])
np.save('./_data/brain/brain_y2_train.npy', arr = xy_train[0][1])
# np.save('./_data/brain/brain_y_tranin.npy', arry = xy_train[0])#xy가 전체 들어가지만 빼서 쓸때 나눠서 빼줘야함
np.save('./_data/brain/brain_x2_test.npy', arr = xy_test[0][0])
np.save('./_data/brain/brain_y2_test.npy', arr = xy_test[0][1])
